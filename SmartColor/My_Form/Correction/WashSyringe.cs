...
{
    public partial class WashSyringe : Form
    {
        public WashSyringe()
        {
            InitializeComponent();
        ...
        private async void BtnRecheck_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请输入需要洗针筒的瓶号！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                [SmartColor.My_DataBase.RUN_TABLE.Machine] = "点击母液瓶批量洗针筒启动按钮"
            }, dt);

            // 3. 启动批量洗针筒
            var results = await SmartColor.My_AutomaticModule.WashSyringeRobotTask.EnqueueBatchWashSyringeAsync
                (validBottleNos);
            ...
            {
                Logger.Error($"瓶号{r.BottleNo}洗针筒异常：{r.Message}", r.Exception);
            }
        }
        ...
        private void ShowRetryDialog(List<int> retryBottleNos, int retryCount)
        {
            SmartColor.My_Tool.MessageEventManager.Instance.RequestShowMessage(
                "洗针筒失败",
                $"以下瓶号洗针筒失败：{string.Join(",", retryBottleNos)}\n是否重新洗针筒？",
                async btn =>
                {
                    if (btn == "重试")
                    {
                        var results = await SmartColor.My_AutomaticModule.WashSyringeRobotTask.EnqueueBatchWashSyringeAsync(retryBottleNos);
                        ...
                        if (failed.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                SmartColor.My_Tool.MessageEventManager.Instance.RequestShowMessage(
                                    "自检失败",
                                    $"以下瓶号多次洗针筒失败：{string.Join(",", failed)}\n请检查针筒密封圈！",
                                    null,
                                    new[] { "确定" },
                                    "确定"
                                );
