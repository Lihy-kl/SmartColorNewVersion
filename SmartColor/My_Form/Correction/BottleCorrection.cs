...
{
    public partial class BottleCorrection : Form
    {
        public BottleCorrection()
        {
            InitializeComponent();
        }
        ...
        private async void BtnCorrection_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请输入需要校正的瓶号！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                [SmartColor.My_DataBase.RUN_TABLE.Machine] = "点击母液瓶校正启动按钮"
            }, dt);

            // 3. 启动批量校正
            var results = await BottleCorrectionRobotTask.EnqueueBatchBottleCorrectionAsync(validBottleNos);
            ...
            {
                Logger.Error($"瓶号{r.BottleNo}校正异常：{r.Message}", r.Exception);
            }
        }
        ...
        private void ShowRetryDialog(List<int> retryBottleNos, int retryCount)
        {
            SmartColor.My_Tool.MessageEventManager.Instance.RequestShowMessage(
                "校正失败",
                $"以下瓶号校正失败：{string.Join(",", retryBottleNos)}\n是否重新校正？",
                async btn =>
                {
                    if (btn == "重试")
                    {
                        var results = await BottleCorrectionRobotTask.EnqueueBatchBottleCorrectionAsync(retryBottleNos);
