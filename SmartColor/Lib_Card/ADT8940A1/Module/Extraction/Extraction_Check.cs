...
namespace Lib_Card.ADT8940A1.Module.Extraction
{
    ...
    /// 针检抽液
    /// </summary>
    public class Extraction_Check : Extraction
    {
        public override int FluidExtraction(int iCylinderVersion, int iSyringeType, int iPulse, int iIsAssitant)
        {
            ...
            int iZRes = CardObject.OA1Axis.Absolute_Z(iSyringeType, SmartColor.My_ConPar.Other.Z_UpPulse, 0);
            if (0 != iZRes)
                return iZRes;

            try
            {
                iZRes = CardObject.OA1Axis.Absolute_Z(iSyringeType, SmartColor.My_ConPar.Other.Z_DownPulse, 0);
                if (0 != iZRes)
                    return iZRes;
            }
            catch (Exception ex)
            ...
            int iTotalPulse = iPulse - SmartColor.My_ConPar.Other.Z_BackPulse;
            iZRes = CardObject.OA1Axis.Absolute_Z(iSyringeType, iTotalPulse, 0);
            if (0 != iZRes)
                return iZRes;
