...
using SmartColor.My_ConPar.Area.BottleArea;
...
namespace SmartColor.My_Cup
{
    ...
    /// </summary>
    internal class FC_12_Send_Base
    {
        ...
        /// <summary>1#AB杯选择（0=双杯，1=主杯，2=副杯）</summary>
        public ProtocolItem CupSelect { get; set; }
    }
    ...
    /// </summary>
    internal class FC_12_Send_Param
    {
    ...
    /// <summary>
    /// 1#发送区-盖子状态区（117~118）
    /// 仅包含A/B盖子状态，便于单独同步盖子状态到HMI。
    /// </summary>
    internal class FC_12_Send_Cover
    {
        /// <summary>1#A盖子状态（0=无，1=有盖 2 = 无盖）</summary>
        public ProtocolItem CoverStatus1 { get; set; }
        /// <summary>1#B盖子状态（0=无，1=有盖 2 = 无盖）</summary>
        public ProtocolItem CoverStatus2 { get; set; }
    }

    internal class FC_12_Send_Pause
    {
        /// <summary>
        /// 暂停信号（1=暂停，2=恢复）
        /// </summary>
    ...
    /// </summary>
    internal class FC_12_Send
    {
        /// <summary>基础控制区（100~106）</summary>
        public FC_12_Send_Base Base { get; set; } = new FC_12_Send_Base();
        /// <summary>工艺参数区（107~116）</summary>
        public FC_12_Send_Param Param { get; set; } = new FC_12_Send_Param();
        /// <summary>盖子状态区（117~118）</summary>
        public FC_12_Send_Cover Cover { get; set; } = new FC_12_Send_Cover();
    }

    /// <summary>
    /// 12杯翻转缸单分区（每2杯为一组）接收区协议结构体。
    /// 每个分区对应20个寄存器，涵盖主/副杯的实时状态、温度、工艺、报警等信息。
    /// 字段说明：
    /// - 主/副杯：每组包含2个杯位（如1-2区、3-4区等），主杯为奇数号，副杯为偶数号。
    /// - 每个ProtocolItem的Address为PLC寄存器地址，Value为实时值。
    /// </summary>
    internal class FC_12_Receive_Zone
    {
        ...
        public ProtocolItem ActualTemp1 { get; set; }
        /// <summary>副杯实际温度（单位0.1℃，如251=25.1℃）</summary>
        public ProtocolItem ActualTemp2 { get; set; }
        /// <summary>当前工艺代码（1=冷行，2=温控，3=加药，4=放布，5=出布，6=排液，8=加水，9=搅拌，12=取小样，13=测PH）</summary>
        public ProtocolItem CurrentProcess { get; set; }
        /// <summary>主杯当前状态（0=待机 1=运行中 2=暂停 3=保温运行 4=排水 5=滴液 6=停止中  7=下线  8=冷行降温）</summary>
        public ProtocolItem CurrentStatus1 { get; set; }
        /// <summary>副杯当前状态（0=待机 1=运行中 2=暂停 3=保温运行 4=排水 5=滴液 6=停止中  7=下线  8=冷行降温）</summary>
        public ProtocolItem CurrentStatus2 { get; set; }
        /// <summary>当前步号（当前工艺流程的步号）</summary>
        public ProtocolItem CurrentStepNum { get; set; }
        /// <summary>当前保温时间（单位：分钟，当前工艺步的保温剩余时间）</summary>
        public ProtocolItem CurrentKeepWarmTime { get; set; }
        /// <summary>主杯盖交互信号（0=无，1=需开盖，2=需关盖，4=申请加药）</summary>
        public ProtocolItem CupCoverSignal1 { get; set; }
        /// <summary>副杯盖交互信号（0=无，1=需开盖，2=需关盖，4=申请加药）</summary>
        public ProtocolItem CupCoverSignal2 { get; set; }
        /// <summary>主杯历史状态（上一次工艺状态，便于判断是否需前洗杯等）</summary>
        public ProtocolItem HistoryStatus1 { get; set; }
        /// <summary>副杯历史状态（上一次工艺状态，便于判断是否需前洗杯等）</summary>
        public ProtocolItem HistoryStatus2 { get; set; }
        ...
        /// <summary>备用3（预留扩展，当前无实际用途）</summary>
        ...
        public FC_12_Receive_Zone(int baseAddr)
        {
            WaitData = new ProtocolItem(baseAddr + 0, typeof(short));
            AllFinish = new ProtocolItem(baseAddr + 1, typeof(short));
            ActualTemp1 = new ProtocolItem(baseAddr + 2, typeof(short));
            ActualTemp2 = new ProtocolItem(baseAddr + 3, typeof(short));
            CurrentProcess = new ProtocolItem(baseAddr + 4, typeof(short));
            CurrentStatus1 = new ProtocolItem(baseAddr + 5, typeof(short));
            CurrentStatus2 = new ProtocolItem(baseAddr + 6, typeof(short));
            CurrentStepNum = new ProtocolItem(baseAddr + 7, typeof(short));
            CurrentKeepWarmTime = new ProtocolItem(baseAddr + 8, typeof(short));
            CupCoverSignal1 = new ProtocolItem(baseAddr + 9, typeof(short));
            CupCoverSignal2 = new ProtocolItem(baseAddr + 10, typeof(short));
            HistoryStatus1 = new ProtocolItem(baseAddr + 11, typeof(short));
            HistoryStatus2 = new ProtocolItem(baseAddr + 12, typeof(short));
            ...
            Reserved3 = new ProtocolItem(baseAddr + 19, typeof(short));
        }
    }

    /// <summary>
    /// 12杯翻转缸所有接收区协议结构体（分区封装，每2杯为一组，共6组）。
    /// 每组对应20个寄存器，涵盖主/副杯的实时状态、温度、工艺、报警等信息。
    /// 便于批量读取、遍历和维护。
    /// </summary>
    internal class FC_12_Receive_All
    {
        /// <summary>1-2区（500~519），主/副杯1/2的所有接收数据</summary>
        public FC_12_Receive_Zone Zone1_2 { get; set; } = new FC_12_Receive_Zone(500);
        /// <summary>3-4区（520~539），主/副杯3/4的所有接收数据</summary>
        public FC_12_Receive_Zone Zone3_4 { get; set; } = new FC_12_Receive_Zone(520);
        /// <summary>5-6区（540~559），主/副杯5/6的所有接收数据</summary>
        public FC_12_Receive_Zone Zone5_6 { get; set; } = new FC_12_Receive_Zone(540);
        /// <summary>7-8区（560~579），主/副杯7/8的所有接收数据</summary>
        public FC_12_Receive_Zone Zone7_8 { get; set; } = new FC_12_Receive_Zone(560);
        /// <summary>9-10区（580~599），主/副杯9/10的所有接收数据</summary>
        public FC_12_Receive_Zone Zone9_10 { get; set; } = new FC_12_Receive_Zone(580);
        /// <summary>11-12区（600~619），主/副杯11/12的所有接收数据</summary>
        public FC_12_Receive_Zone Zone11_12 { get; set; } = new FC_12_Receive_Zone(600);


        public FC_12_Receive_Zone GetZoneFromGroup(int group)
        {

            switch (group)
            {
                case 0: return Zone1_2;
                case 1: return Zone3_4;
                case 2: return Zone5_6;
                case 3: return Zone7_8;
                case 4: return Zone9_10;
                case 5: return Zone11_12;
                default: throw new ArgumentOutOfRangeException($"无效的组号: {group}");
            }
        }

        public FC_12_Receive_Zone GetZoneFromCupNo(int cupNo, int startCupNo)
        {
            if (cupNo == 0)
                throw new Exception($"无效的组号: {cupNo}");
            int group = (cupNo - startCupNo) / 2;
    ...
    /// <summary>
    /// 12杯翻转缸通讯协议主类，支持威纶通/昆仑通态两种HMI类型
    /// </summary>
    internal class FC_12 : ICylinderComm
    {
        ...
        private readonly FC_12_Receive_All _receiveAll = new FC_12_Receive_All();

        private readonly FC_12_Send_Pause _pause = new FC_12_Send_Pause();

        /// <summary>
        /// 版本区协议项集合
        /// </summary>
        private readonly FC_12_Version _version = CreateVersionInfo();
        ...
        private readonly ConcurrentDictionary<int, Tuple<int, string>> _lastStepSent = new ConcurrentDictionary<int, Tuple<int, string>>();
        ...
        public FC_12(string ip, int port, int startCupNo, int hmiType, string areaName)
        {
        ...
        private static FC_12_Send CreateSend(int baseAddr)
        {

            var send = new FC_12_Send
            {
                Base = new FC_12_Send_Base
                {
                    ...
                    CupSelect = new ProtocolItem(baseAddr + 6, typeof(ushort))
                },
                Param = new FC_12_Send_Param
                {
                ...
                Cover = new FC_12_Send_Cover
                {
                    CoverStatus1 = new ProtocolItem(baseAddr + 17, typeof(ushort)),
                    CoverStatus2 = new ProtocolItem(baseAddr + 18, typeof(ushort))
                }
            };
            return send;
        }
        ...
        private static FC_12_Version CreateVersionInfo()
        {
            return new FC_12_Version
            {
        ...
        /// 获取所有发送区协议项集合
        /// </summary>
        public List<FC_12_Send> GetSendList() => _send;

        /// <summary>
        /// 获取版本区协议项
        /// </summary>
        public FC_12_Version GetVersionInfo() => _version;
        ...
        private static int GetVersionRegisterCount(FC_12_Version version)
        {
            int count = 0;
            foreach (var prop in typeof(FC_12_Version).GetProperties())
            {
        ...
        /// <param name="send">FC_12_Send结构体</param>
        private void FillProcessNameToCodes(string processName, FC_12_Send_Base send)
        {
            // 1. GB2312编码
        ...
        private void ParseAlarmTip(int cupNo, ushort alarmTip, FC_12_Receive_Zone zone)
        {
        ...
        private async Task SyncAllCoverStatus()
        {
            ...
            {
                int cupNum = this._startCupNo + i * 2;
        ...
        private void WaitDataProcess(int cupNo, int currentStepNum, FC_12_Receive_Zone zone)
        {
            // 防止同一杯号并发处理
            ...
            try
            {
                ...
                // 2. 杯选择
                ushort cupSelect = My_Tool.CupAuxiliary.GetCupChioce(cupNo);
                ...
                if (isWash)
                {
                    ...
                    int haveCloth2 = subInfo.HaveCloth ?? 0;
                    if (nextProcess.TechnologyName == "出布")
                    {
                        if (cupSelect == 0 || cupSelect == 1)
                            haveCloth1 = mainInfo.HaveCloth ?? 0;
                        if (cupSelect == 0 || cupSelect == 2)
                            haveCloth2 = subInfo.HaveCloth ?? 0;
                        if (haveCloth1 == 0 && haveCloth2 == 0)
                        {
                            //都没有布，则找下一步
                ...
                else
                {
                    ...
                    int subCupHeadID = subInfo.HeadID ?? 0;
                    if (mainCupHeadID == 0 && subCupHeadID == 0)
                    {
                        throw new Exception($"WaitDataProcess: 杯号{cupNo}当前步骤{currentStepNum}主副杯HeadID均为0，无法获取下一步工艺");
                    ...
                    else if (cupSelect == 0 || cupSelect == 2)
                    {
                        headID = subCupHeadID;
                    }
                    else
                    {
                        throw new Exception($"WaitDataProcess: 杯号{cupNo}当前步骤{currentStepNum}杯选择异常，无法确定使用主副杯HeadID");
                ...
                if (cupSelect == 0 || cupSelect == 2)
                {
                    ...
                    var area = CupCommManager.Instance.FindCupAreaByCupNum(subInfo.CupNum);
                    if (area != null)
                    {
        ...
        private async Task CheckNeedRobotTask(int cupNum, CupAuxiliary.StepInfo stepInfo, bool isFrist = false)
        {
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    int group = (mainInfo.CupNum - this._startCupNo) / 2;

                    // 2. 获取分区
                    FC_12_Receive_Zone zone = this._receiveAll.GetZoneFromGroup(group);
                    ...
                    if (technologyName == "冷行" || technologyName == "温控" || technologyName == "搅拌")
                    {
                        ...
                        if (subInfo.IsUsing == 1 && subInfo.CoverStatus == 2)
                        {
                            needCloseCover = true;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                var recorder = SmartColor.My_File.CupTempRecorder.Get(subInfo.CupNum);
                                recorder.SetCylinderComm(this);
                                recorder.RecordStep(stepInfo.TechnologyName);
                            }
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    Logger.Error($"CheckNeedRobotTask: {useName}号杯机械手任务判断异常 step={stepInfo.StepNum}, tech={stepInfo.TechnologyName}", ex);
                }
            });
        }
        ...
        /// </summary>
        private async Task WaitLockAndEnqueue(int cupNo, FC_12_Receive_Zone zone, int dyeType, string technologyName = null)
        {
            // 统一等待锁止信号
            ...
            if (technologyName != null)
            {
                ...
                label1:
                    ...
                    int dd = Convert.ToInt32(zone.DrainageDown.Value);
                    bool readly = false;
                    // 主杯
                    if (Convert.ToInt16(zone.CurrentStatus1.Value) != 7)
                    {
                        double temp = Math.Round(Convert.ToDouble(zone.ActualTemp1.Value) / 10, 1);
                        if (temp < sol)
                        {
                            // 低温
                            readly = true;
                    ...
                    // 副杯
                    if (Convert.ToInt16(zone.CurrentStatus2.Value) != 7)
                    {
                        double temp = Math.Round(Convert.ToDouble(zone.ActualTemp1.Value) / 10, 1);
                        if (temp < sol)
                        {
        ...
        /// </summary>
        /// <param name="cupNo">杯号</param>
        private void AllFinishProcess(int cupNo, FC_12_Receive_Zone zone)
        {
            lock (this._zoneLock)
            {
            ...
            try
            {
                ...
                FC_12_Send_Param send = this._send[group].Param;
                ...
                bool subIsWash = !string.IsNullOrEmpty(subDyeingCode) && CupAuxiliary.WashCupDic.ContainsKey(subDyeingCode) || subDyeingCode == null;

                ushort cupSelect = 0;
                if (mainInfo.IsUsing == 1 && subInfo.IsUsing == 1)
                {
                    cupSelect = 0; // 双杯
                    ...
                    //停止温度记录
                    recorder = SmartColor.My_File.CupTempRecorder.Get(subInfo.CupNum);
                    recorder.SetCylinderComm(this);
                    recorder.StopRecord();
                    if (!mainIsWash)
                    {
                        if (this._retrying.Contains(mainCup) || this._retrying.Contains(subCup))
                        {
                            if (this._retrying.Contains(mainCup))
                                this._retrying.Remove(mainCup);
                            else
                                this._retrying.Remove(subCup);
                        }
                        else
                        {
                ...
                else if (mainInfo.IsUsing == 1 && subInfo.IsUsing == 0)
                {
                    cupSelect = 1; // 主杯
                                   //停止温度记录
                    var recorder = SmartColor.My_File.CupTempRecorder.Get(mainInfo.CupNum);
                ...
                else if (mainInfo.IsUsing == 0 && subInfo.IsUsing == 1)
                {
                    cupSelect = 2; // 副杯
                                   //停止温度记录
                    var recorder = SmartColor.My_File.CupTempRecorder.Get(subInfo.CupNum);
                    ...
                    if (!subIsWash)
                    {
                        if (this._retrying.Contains(subCup))
                        {
        ...
        private void CupSignalProcess(int cupNo, int signal, FC_12_Receive_Zone zone)
        {
            ...
            _ = Task.Run(async () =>
              {
                  ...
                  try
                  {
                      ...
                      //复位信号
                      int group = (cupNo - this._startCupNo) / 2;



                      bool mainUsing = mainInfo.IsUsing == 1;
                      ...
                      {
                          int cupNum = mainUsing ? mainInfo.CupNum : subInfo.CupNum;
            ...
            try
            {
                    ...
                    default:
                        {
                            ...
                            else if (subInfo.IsUsing == 1)
                            {
                                confirmValue = new PutClothConfirmResult
                                {
                                    sureKey = 3,
                                    dyeType = subInfo.DyeType ?? 0,
                                    headID = subInfo.HeadID ?? 0,
                                    stepNo = int.TryParse(subInfo.StepNum?.ToString(), out var stepNum) ? stepNum : 0,
                                    dyeingCode = subInfo.DyeingCode ?? ""
                                };
                            }
                            else
                            {
                ...
                switch (confirmValue.sureKey)
                {
                    ...
                    case 2:
                        ...
                        if (tc == "放布")
                            MessageEventManager.Instance.RequestLoopSpeak($"{nm}号配液杯入布", $"{nm}号杯请放布");
                        else if (tc == "出布")
                            MessageEventManager.Instance.RequestLoopSpeak($"{nm}号配液杯出布", $"{nm}号杯请出布");
                    ...
                    case 3:
                        ...
                        if (tc == "出布")
                            MessageEventManager.Instance.RequestStopLoopSpeak($"{nm}号配液杯出布");
                        else if (tc == "放布")
                            MessageEventManager.Instance.RequestStopLoopSpeak($"{nm}号配液杯入布");
                        else if (tc == "取小样")
                            MessageEventManager.Instance.RequestStopLoopSpeak($"{nm}号配液杯取小样");

                        else if (tc == "测PH")
                            MessageEventManager.Instance.RequestStopLoopSpeak($"{nm}号配液杯测PH");
                        break;
                }
        ...
        /// </summary>
        /// <param name="cupNo">杯号</param>
        private void ModuleRestartProcess(int cupNo, FC_12_Receive_Zone zone)
        {
            ...
            try
            {
                ...
                Logger.Error($"{mainInfo.CupNum}/{subInfo.CupNum}号杯收到重启信号");
                if (mainInfo.IsUsing == 1 || subInfo.IsUsing == 1)
                {
                    lock (this._zoneLock)
                    {
                    ...
                    var useinfo = mainInfo.IsUsing == 1 ? mainInfo : subInfo;
        ...
        private int ReadAllReceive(bool onlyRead)
        {
            ...
            try
            {
                lock (_zoneLock)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        // Console.WriteLine($"{DateTime.Now}：开始读取工位 {i} 的数据");
                ...
                // Console.WriteLine($"{DateTime.Now}：一次轮询结束");
        ...
        /// <summary>
        /// 处理所有接收区数据，将每组（每2杯）数据分发到主/副杯，并触发数据接收事件。
        /// 包括：
        /// 1. 推送主/副杯主要数据到上层（如界面/业务层）
        /// 2. 解析报警提示，输出日志
        /// 3. 处理等待数据、全部完成、机械手信号、放布确认、模块重启等业务指令
        /// </summary>
        private async Task ReceiveDataProcess(bool onlyRead, FC_12_Receive_Zone zone, int group)
        {
            await Task.Run(() =>
            {
                // 计算主/副杯的全局索引（如1/2、3/4...）
                int cupIndex1 = this._startCupNo + group * 2;
                int cupIndex2 = cupIndex1 + 1;

                // 1. 推送主杯数据
                var cupData1 = new Dictionary<string, object>
            {
                { "CupIndex", cupIndex1 },
                { "CurrentStatus", ConvertCodeToStatues(Convert.ToUInt16(zone.CurrentStatus1.Value)) },
                { "ActualTemp", zone.ActualTemp1.Value },
            ...
            {
                { "CupIndex", cupIndex2 },
                { "CurrentStatus", ConvertCodeToStatues(Convert.ToUInt16(zone.CurrentStatus2.Value)) },
                { "ActualTemp", zone.ActualTemp2.Value },
                { "LockSignal", zone.LockUpStatus.Value },
                { "CurrentStepNo", zone.CurrentStepNum.Value },
                { "HoldingTime", zone.CurrentKeepWarmTime.Value },
                { "TechnologyName", ConvertCodeToTechnologyName(Convert.ToUInt16(zone.CurrentProcess.Value)) },
                ...
                OnCupDataReceived?.Invoke(cupIndex2, cupData2);
                ...
                ushort cupCoverSignal1 = Convert.ToUInt16(zone.CupCoverSignal1.Value);
                ushort cupCoverSignal2 = Convert.ToUInt16(zone.CupCoverSignal2.Value);
                ...
                ushort historyStatus1 = Convert.ToUInt16(zone.HistoryStatus1.Value);
                ushort historyStatus2 = Convert.ToUInt16(zone.HistoryStatus2.Value);
                ...
                // 9. 历史状态处理
                ushort capturedHistorySignal = (historyStatus1 == 1 || historyStatus2 == 1) ? (ushort)1 : (ushort)0;
        ...
        private void PutOrOutCloth(int cupIndex1, ushort signal, FC_12_Receive_Zone zone)
        {
            ...
            _ = Task.Run(async () =>
             {
                 try
                 {
                     FC_12_Receive_Zone zone = this._receiveAll.GetZoneFromCupNo(cupNo, this._startCupNo);
                     if ((Convert.ToInt16(zone.HistoryStatus1.Value) == 1 && Convert.ToInt16(zone.CurrentStatus1.Value) == 0) ||
                        (Convert.ToInt16(zone.HistoryStatus2.Value) == 1 && Convert.ToInt16(zone.CurrentStatus2.Value) == 0))
                ...
                while (retryCount < maxRetry)
                {
                    try
                    {
                        ...
                        for (int group = 0; group < 6; group++)
                        {
                            try
                            {
                                int cupIndex1 = this._startCupNo + group * 2; // 主杯
                                FC_12_Receive_Zone zone = this._receiveAll.GetZoneFromGroup(group);
                                ...
                                if (mainInfo.IsUsing == 1 || subInfo.IsUsing == 1)
                                {
                                    ...
                                    else if (useinfo.DyeingCode == My_Tool.CupAuxiliary.PreWashCupType)
                                    {
                                        ...
                                        CupAuxiliary.ClearBatchCupData(subInfo.CupNum);
                            ...
                            {
                                Logger.Error($"启动后补偿杯号{this._startCupNo + group * 2}异常", ex);
                            }
                        }
        ...
        public async Task<DyeingResult> RequestDropLiquidAsync(int cupNum, int cupChoice)
        {
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    {
                        CupTempRecorder.ClearFiles(subInfo.CupNum);
                    }

                    // 2. 读取主/副杯当前状态和历史状态
                    ushort mainCurrentStatus = Convert.ToUInt16(zone.CurrentStatus1.Value);
                    ushort mainHistoryStatus = Convert.ToUInt16(zone.HistoryStatus1.Value);
                    ushort subCurrentStatus = Convert.ToUInt16(zone.CurrentStatus2.Value);
                    ushort subHistoryStatus = Convert.ToUInt16(zone.HistoryStatus2.Value);



                    var mainHaveCloth = Convert.ToUInt16(mainInfo.HaveCloth);
                    var subHaveCloth = Convert.ToUInt16(subInfo.HaveCloth);

                    // 当前杯的状态和历史状态
                    ushort currentStatus = isMainCup ? mainCurrentStatus : subCurrentStatus;
                    ushort historyStatus = isMainCup ? mainHistoryStatus : subHistoryStatus;
                    ...
                    bool needPreWash = (mainCurrentStatus == 0 && (mainHistoryStatus == 1 || mainHaveCloth == 1 || mainInfo.CoverStatus == 1)) ||
                        (subCurrentStatus == 0 && (subHistoryStatus == 1 || subHaveCloth == 1 || subInfo.CoverStatus == 1));
                    ...
                    while (waitMs < maxWaitMs)
                    {
                        lock (this._zoneLock)
                        {
                            ...
                            send.Base.CupSelect.SetValue(cupChoice); // 0=双杯, 1=主杯, 2=副杯
        ...
        public async Task<DyeingResult> DyeingStartAsync(int cupNum, int cupChoice)
        {
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    {
                        var filter = $"{WAIT_LIST.CupNum} = {otherInfo.CupNum}";
                        var datarows = SqlServer.Select(WAIT_LIST.TableName, filter);
                        if (datarows != null && datarows.Rows.Count > 0)
                        {
                            result.Code = My_Tool.Result.ResultCode.Failure;
                            result.Message = "等待列表中存在另一杯的待滴液任务";
                            return result;
                        }
                    }

                    var dropHeadDt = SqlServer.Select(DROP_HEAD.TableName, $"{DROP_HEAD.CupNum} = {otherInfo.CupNum} AND {DROP_HEAD.CupFinish} = 0");

                    if (dropHeadDt.Rows.Count > 0)
                    {
                        result.Code = My_Tool.Result.ResultCode.Failure;
                        result.Message = $"杯号{otherInfo.CupNum}已存在滴液任务";
                        return result;
                    }
                    ...
                    if (mainInfo.IsUsing == 1)
                    {
                        var states1 = Convert.ToUInt16(zone.CurrentStatus1.Value);
                    ...
                    if (subInfo.IsUsing == 1)
                    {
                        var states2 = Convert.ToUInt16(zone.CurrentStatus2.Value);
                        if (states2 != 0 && states2 != 5)
                            subOk = false;
                    ...
                    var info = mainInfo.CupNum == cupNum ? mainInfo : subInfo;
                    ...
                    {
                        string where = $"{CUP_DETAILS.CupNum}=@CupNum";
                        var whereParam = new SqlParameter("@CupNum", subInfo.CupNum);
                        My_DataBase.SqlServer.Update(CUP_DETAILS.TableName, updateDict, where, whereParam);
        ...
        public double GetCupTemp(int cupNum)
        {
            ...
            FC_12_Receive_Zone zone = this._receiveAll.GetZoneFromCupNo(cupNum, this._startCupNo);
            return isMainCup ? Convert.ToInt16(zone.ActualTemp1.Value) / 10 : Convert.ToInt16(zone.ActualTemp2.Value) / 10;
        }
        ...
        public void RequestStop()
        {
            ...
            _ = RunTableMan.InsertAsync(new Dictionary<string, object>
            {
                [SmartColor.My_DataBase.RUN_TABLE.Dail] = $"{this._startCupNo}号杯所属机台启动停止通讯指令"
            }, dt);
        }



        public async Task SendStopAsync(int cupNum, bool needOffLine)
        {
            ...
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    lock (this._zoneLock)
                    {
                        ...
                        send.Base.TotalStepNum?.SetValue(0);
                        FillProcessNameToCodes(string.Empty, send.Base);
                        send.Base.CupSelect?.SetValue(0);
                        ResetSendParamToZero(send.Param);
                        WriteToHMI(send.Base);
                    }

                    var mscup = My_Tool.CupAuxiliary.GetCupPair(cupNum);
                    var msCups = My_Tool.CupAuxiliary.GetIsUseing(cupNum);
                    ...
                    while (waitMs < maxWaitMs)
                    {
                        ...
                        var status2 = subInfo.Statues;

                        // 检查主/副杯是否都已进入停止、待机或下线
                        bool mainOk = status1 == "待机" || status1 == "下线";
                        bool subOk = status2 == "待机" || status2 == "下线";
                    ...
                    else
                    {
                        ...
                        My_DataBase.SqlServer.Update(
                          ...
                          $"{CUP_DETAILS.CupNum} = @cupNo",
                          new SqlParameter("@cupNo", subInfo.CupNum)
                      );

                        area = CupCommManager.Instance.FindCupAreaByCupNum(subInfo.CupNum);
                        if (area != null)
                        {
                            area.OnCupDataReceived(subInfo.CupNum);
                        }
        ...
        public async Task SendWashAsync(int cupNum, string type)
        {
            await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    // 4. 用GetMSCupInfo获取主副杯状态
                    int cupSelect = 0;
                    var (mainInfo, subInfo) = CupAuxiliary.GetMSCupInfo(cupNum);
                    if (name == CupAuxiliary.FailWashCupType)
                    {
                        if (mainInfo.IsUsing == 1 && subInfo.IsUsing == 0)
                            cupSelect = 1;
                        else if (mainInfo.IsUsing == 0 && subInfo.IsUsing == 1)
                            cupSelect = 2;
                        else if (mainInfo.IsUsing == 1 && subInfo.IsUsing == 1)
                            cupSelect = 0;
                        else
                            cupSelect = 0;
                    }
                    ...
                    {
                        if ((Convert.ToInt16(zone.CurrentStatus1.Value) == 0 && Convert.ToInt16(zone.CurrentStatus2.Value) == 0))
                            cupSelect = 0;
                        else if (Convert.ToInt16(zone.CurrentStatus1.Value) == 0 && Convert.ToInt16(zone.CurrentStatus2.Value) == 7)
                            cupSelect = 1;
                        else if (Convert.ToInt16(zone.CurrentStatus1.Value) == 7 && Convert.ToInt16(zone.CurrentStatus2.Value) == 0)
                            cupSelect = 2;
                        else
                        {
                            return;
                    ...
                    send.Base.CupSelect.Value = cupSelect;
                    send.Base.StartStopReady.Value = 1;

                    // 5. 查找总浴量
                    double totalWeight = 0;
                    var cupArea = SmartColor.My_AutomaticModule.CupCommManager.Instance.FindCupAreaByCupNum(mainInfo.CupNum);
                    ...
                    _ = RunTableMan.InsertAsync(new Dictionary<string, object>
                    {
                        [SmartColor.My_DataBase.RUN_TABLE.Dail] = $"{useName}号杯启动{type}指令"
                    }, dt);
        ...
        public async Task SendOffLine(int cupNum, int cupChoice)
        {
            await Task.Run(() =>
            {
                ...
                {
                    var (mainInfo, subInfo) = CupAuxiliary.GetMSCupInfo(cupNum);
                    if (mainInfo.CupNum != subInfo.CupNum)
                    {
                        var otherInfo = mainInfo.CupNum == cupNum ? subInfo : mainInfo;
                        if (otherInfo.Statues != "待机" && otherInfo.Statues != "下线")
                        {
                            Logger.Error("另一杯正在运行，禁止下线当前杯", new Exception($"主杯{mainInfo.CupNum}使用状态{mainInfo.IsUsing},副杯{subInfo.CupNum}使用状态{subInfo.IsUsing}"));
                            return;
                        }
                    }
        ...
        /// <summary>
        /// 将FC_12_Send_Param所有协议项的值置为0
        /// </summary>
        /// <param name="param">工艺参数区对象</param>
        private void ResetSendParamToZero(FC_12_Send_Param param)
        {
            lock (this._zoneLock)
            {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    if (mainInfo.CupNum != subInfo.CupNum)
                    {
                        ...
                        {
                            Logger.Error("另一杯正在运行，禁止上线当前杯", new Exception($"主杯{mainInfo.CupNum}使用状态{mainInfo.IsUsing},副杯{subInfo.CupNum}使用状态{subInfo.IsUsing}"));
                            return;
                        }
                    }
...

                    {
                        SqlServer.Update(CUP_DETAILS.TableName,
                            ...
                            $"{CUP_DETAILS.CupNum} = {mainInfo.CupNum}");
                    ...
                    if (cupChoice == 0 || cupChoice == 2)
                    {
                        SqlServer.Update(CUP_DETAILS.TableName,
                           ...
                           $"{CUP_DETAILS.CupNum} = {subInfo.CupNum}");
        ...
        public async Task SyncCoverStatus(int cupNum)
        {
            await Task.Run(() =>
            {
                ...
                {
                    var (mainInfo, subInfo) = CupAuxiliary.GetMSCupInfo(cupNum);
                    var sendIdx = (cupNum - this._startCupNo) / 2;

                    ushort coverStatus1 = (ushort)(mainInfo.CoverStatus ?? 0);
                    ushort coverStatus2 = (ushort)(subInfo.CoverStatus ?? 0);

                    this._send[sendIdx].Cover.CoverStatus1.Value = coverStatus1;
                    this._send[sendIdx].Cover.CoverStatus2.Value = coverStatus2;
            ...
            try
            {
                    ...
                    if (!subIsWash)
                    {
                        if (prevStepNum >= 0)
                        {
                            My_DataBase.SqlServer.Update(DYE_DETAILS.TableName,
                                ...
                                $"{DYE_DETAILS.HeadID} = @headID AND {DYE_DETAILS.StepNum} = @currentStepNum ",
                                new SqlParameter("@headID", subInfo.HeadID),
                                new SqlParameter("@currentStepNum", prevStepNum)
                            );
                        }
                        My_DataBase.SqlServer.Update(DYE_DETAILS.TableName,
                ...
                while (waitMs < maxWaitMs)
                {
                    ...
                    for (int i = 0; i < 5; i++)
                    {
                        await Task.Delay(pollInterval);
                        waitMs += pollInterval;
                        var sn = Convert.ToUInt16(zone.CurrentStepNum?.Value ?? 0);
                        var tn = Convert.ToUInt16(zone.CurrentProcess?.Value ?? 0);
                        var (mainInfo2, subInfo2) = CupAuxiliary.GetMSCupInfo(cupNum);
                        bool mainUsing = mainInfo2.IsUsing == 1;
                        bool subUsing = subInfo2.IsUsing == 1;

                        if (mainInfo2.Enable == 0 && subInfo2.Enable == 0)
                        {
                            return;
                        }

                        bool mainOk = true, subOk = true;
                        if (mainUsing)
                        {
                            ushort dbStepNum = sn;
                        ...
                        if (subUsing)
                        {
                            ...
                            ushort dbProcess = tn;
                            subOk = (dbStepNum == targetStepNum && dbProcess == targetProcess);
                        }


                        if ((mainUsing && !subUsing && mainOk) ||
                            (!mainUsing && subUsing && subOk) ||
                            (mainUsing && subUsing && mainOk && subOk))
                        {
                            sure = true;
                            break;
                        }
