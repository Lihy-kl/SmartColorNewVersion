...
namespace SmartColor.My_Cup
{
    ...
    /// </summary>
    internal class FC_6_Send_Param
    {
    ...
    /// </summary>
    internal class FC_6_Send_Cover
    {
    ...
    internal class FC_6_Send_Pause
    {
        /// <summary>
        /// 暂停信号（1=暂停，2=恢复）
        /// </summary>
    ...
    /// </summary>
    internal class FC_6_Send
    {
        /// <summary>基础控制区（292~298）</summary>
        public FC_6_Send_Base Base { get; set; } = new FC_6_Send_Base();
        /// <summary>工艺参数区（299~308）</summary>
        public FC_6_Send_Param Param { get; set; } = new FC_6_Send_Param();
        /// <summary>盖子状态区（309）</summary>
        public FC_6_Send_Cover Cover { get; set; } = new FC_6_Send_Cover();
    }
    ...
    /// </summary>
    internal class FC_6_Receive_Zone
    {
        ...
        public FC_6_Receive_Zone(int baseAddr)
        {
    ...
    /// 便于批量读取、遍历和维护。
    /// </summary>
    internal class FC_6_Receive_All
    {
        /// <summary>1区（500~519），1杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone1 { get; set; } = new FC_6_Receive_Zone(500);
        /// <summary>2区（520~539），2杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone2 { get; set; } = new FC_6_Receive_Zone(520);
        /// <summary>3区（540~559），3杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone3 { get; set; } = new FC_6_Receive_Zone(540);
        /// <summary>4区（560~579），4杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone4 { get; set; } = new FC_6_Receive_Zone(560);
        /// <summary>5区（580~599），5杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone5 { get; set; } = new FC_6_Receive_Zone(580);
        /// <summary>6区（600~619），6杯所有接收数据</summary>
        public FC_6_Receive_Zone Zone6 { get; set; } = new FC_6_Receive_Zone(600);


        public FC_6_Receive_Zone GetZoneFromGroup(int group)
        {
        ...
        public FC_6_Receive_Zone GetZoneFromCupNo(int cupNo, int startCupNo)
        {
    ...
    /// <summary>
    /// 翻转缸6杯位通讯协议主类，包含所有协议项集合及工厂方法。
    /// 支持PLC通讯、数据解析，并通过事件推送每个杯位的数据到UI层。
    /// </summary>
    internal class FC_6 : ICylinderComm
    {
        ...
        /// 所有杯位的发送区协议项集合
        /// </summary>
        private readonly List<FC_6_Send> _send = new List<FC_6_Send>()
        {
            CreateSend(100),
            CreateSend(164),
            CreateSend(228),
        ...
        private readonly FC_6_Receive_All _receiveAll = new FC_6_Receive_All();

        private readonly FC_6_Send_Pause _pause = new FC_6_Send_Pause();

        /// <summary>
        /// 版本区协议项集合
        /// </summary>
        private readonly FC_6_Version _version = CreateVersionInfo();
        ...
        public FC_6(string ip, int port, int startCupNo, int hmiType, string areaName)
        {
        ...
        private static FC_6_Send CreateSend(int baseAddr)
        {

            var send = new FC_6_Send
            {
                Base = new FC_6_Send_Base
                {
        ...
        private static FC_6_Version CreateVersionInfo()
        {
            return new FC_6_Version
            {
        ...
        /// 获取所有发送区协议项集合
        /// </summary>
        public List<FC_6_Send> GetSendList() => _send;

        /// <summary>
        /// 获取版本区协议项
        /// </summary>
        public FC_6_Version GetVersionInfo() => _version;
        ...
        private static int GetVersionRegisterCount(FC_6_Version version)
        {
            int count = 0;
            foreach (var prop in typeof(FC_6_Version).GetProperties())
            {
        ...
        /// <param name="send">FC_6_Send结构体</param>
        private void FillProcessNameToCodes(string processName, FC_6_Send_Base send)
        {
            // 1. GB2312编码
        ...
        private void ParseAlarmTip(int cupNo, ushort alarmTip, FC_6_Receive_Zone zone)
        {
        ...
        private void WaitDataProcess(int cupNo, int currentStepNum, FC_6_Receive_Zone zone)
        {
            // 防止同一杯号并发处理
        ...
        private async Task CheckNeedRobotTask(int cupNum, CupAuxiliary.StepInfo stepInfo, bool isFrist = false)
        {
            await Task.Run(async () =>
            {
                try
                {
                    ...
                    // 2. 获取分区
                    FC_6_Receive_Zone zone = this._receiveAll.GetZoneFromGroup(group);
                    // 4. 获取dyeType
        ...
        /// </summary>
        private async Task WaitLockAndEnqueue(int cupNo, FC_6_Receive_Zone zone, int dyeType, string technologyName = null)
        {
            // 统一等待锁止信号
        ...
        /// </summary>
        /// <param name="cupNo">杯号</param>
        private void AllFinishProcess(int cupNo, FC_6_Receive_Zone zone)
        {
            ...
            try
            {
                ...
                FC_6_Send_Param send = this._send[group].Param;
        ...
        /// <summary>
        /// 将FC_6_Send_Param所有协议项的值置为0
        /// </summary>
        /// <param name="param">工艺参数区对象</param>
        private void ResetSendParamToZero(FC_6_Send_Param param)
        {
            lock (this._zoneLock)
            {
        ...
        private void CupSignalProcess(int cupNo, int signal, FC_6_Receive_Zone zone)
        {
        ...
        /// </summary>
        /// <param name="cupNo">杯号</param>
        private void ModuleRestartProcess(int cupNo, FC_6_Receive_Zone zone)
        {
        ...
        /// </summary>
        private async Task ReceiveDataProcess(bool onlyRead, FC_6_Receive_Zone zone, int group)
        {
        ...
        private void PutOrOutCloth(int cupIndex1, ushort signal, FC_6_Receive_Zone zone)
        {
        ...
        private void HistoryStateSendWashAsync(int cupNo)
        {
            _ = Task.Run(async () =>
            {
                try
                {
                    FC_6_Receive_Zone zone = this._receiveAll.GetZoneFromCupNo(cupNo, this._startCupNo);
        ...
        public async Task<DyeingResult> DyeingStartAsync(int cupNum, int cupChoice)
        {
            return await Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    _ = RunTableMan.InsertAsync(new Dictionary<string, object>
                    {
                        [SmartColor.My_DataBase.RUN_TABLE.Dail] = $"{cupNum}号杯启动染固色指令"
                    }, dt);
                }
                catch (Exception ex)
                {
        ...
        {
            FC_6_Receive_Zone zone = this._receiveAll.GetZoneFromCupNo(cupNum, this._startCupNo);
            ...
            return await Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    if (needPreWash)
                    {
                        ...
                        result.Message = $"{cupNum}号杯历史状态为1，需前洗杯";
                        return result;
